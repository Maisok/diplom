                ...
                <div x-show="activeTab === 'equipment'" class="pt-4">
                    <div class="grid grid-cols-1 gap-2">
                        @foreach([
                            ...
                            ['title' => 'Макс. скорость', 'value' => $car->equipment->max_speed ? $car->equipment->max_speed.' км/ч' : '—'],
                            ['title' => 'Емкость батареи', 'value' => $car->equipment->battery_capacity ? $car->equipment->battery_capacity.' кВт·ч' : '—'],
                            ['title' => 'Запас хода', 'value' => $car->equipment->range ? $car->equipment->range.' км' : '—'],
                        ] as $item)
                               @if($item['value'] != '—')
...
<script>
    ...
    const renderer = new THREE.WebGLRenderer({ 
        canvas: canvas,
        antialias: true,
        alpha: false // ❗ фон непрозрачный
    });
    renderer.setSize(canvas.clientWidth, canvas.clientHeight);
    renderer.setClearColor(0x1a1a1a, 1); // Темно-серый фон для контраста
    renderer.physicallyCorrectLights = true;
    renderer.outputEncoding = THREE.sRGBEncoding; // Важно для правильного отображения цветов
    renderer.toneMapping = THREE.ACESFilmicToneMapping; // Лучший контраст
    ...
    // Добавляем HDRI окружение (рефлексы и блики)
    ...
    loader.load(modelUrl, function(gltf) {
        ...
        model.traverse(child => {
            if (child.isMesh && child.material) {
                // Если нужно изменить материалы вручную:
                if (child.material.map) {
                    child.material.map.colorSpace = THREE.SRGBColorSpace; // корректная цветовая гамма
                }
                if (child.material.emissiveMap) {
                    child.material.emissiveMap.colorSpace = THREE.SRGBColorSpace;
                }

                // Можно немного усилить металлик/глянец
